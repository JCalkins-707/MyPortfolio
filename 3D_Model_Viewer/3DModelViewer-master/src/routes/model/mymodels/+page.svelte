...
{#if modelsList.results.length > 0}
    <ul>
        {#each modelsList.results as result}
            <li>
                <ModelCard modelTitle={result.modelTitle}
                modelAuthor={result.modelAuthor} modelId={result.modelId}
                isAuthor={true} tags={result.tags} modelApproved={result.approved}
                modelDeleted={result.deleted}
                />
            </li>
        {/each}
    </ul>
